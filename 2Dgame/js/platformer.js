...
// Assets
var playerSprites = [new Image(), new Image()];
playerSprites[0].src = "assets/player.png";
playerSprites[1].src = "assets/player2.png";
...
PlatformerNode.prototype = {
  ...
  getXCells(resolution) {
    return {
      start: Math.floor(
        (this.x + PlatformerGrid.prototype.EPSILON) / resolution
      ),
      end: Math.floor(
        (this.x + this.width - PlatformerGrid.prototype.EPSILON) / resolution
      ),
    };
  },
  ...
  getYCells(resolution) {
    return {
      start: Math.floor(
        (this.y + PlatformerGrid.prototype.EPSILON) / resolution
      ),
      end: Math.floor(
        (this.y + this.height - PlatformerGrid.prototype.EPSILON) / resolution
      ),
    };
  },
  ...
  getCellBottom(y, resolution) {
    return Math.floor(
      (y + this.height - PlatformerGrid.prototype.EPSILON) / resolution
    );
  },
  ...
  getCellRight(x, resolution) {
    return Math.floor(
      (x + this.width - PlatformerGrid.prototype.EPSILON) / resolution
    );
  },
...
PlatformerGrid.prototype = {
  ...
  setWall(x, y, wall) {
    if (this.validateCoordinates(x, y)) this.getCell(x, y).wall = wall;
  },

  setCeiling(x, y, ceiling) {
    if (this.validateCoordinates(x, y)) this.getCell(x, y).ceiling = ceiling;
  },
  ...
  update(timeStep) {
    for (var i = 0; i < this.nodes.length; ++i) {
      ...
      if (node.vx != 0) {
        ...
        if (node.vx > 0) {
          if (
            node.getCellRight(node.x, this.resolution) !=
            node.getCellRight(node.xp, this.resolution)
          ) {
            const yCells = node.getYCells(this.resolution);

            for (var y = yCells.start; y <= yCells.end; ++y) {
              if (
                this.getWall(node.getCellRight(node.x, this.resolution), y) ||
                (y != yCells.start &&
                  this.getCeiling(
                    node.getCellRight(node.x, this.resolution),
                    y
          ...
          if (
            node.getCellLeft(node.x, this.resolution) !=
            node.getCellLeft(node.xp, this.resolution)
          ) {
            const yCells = node.getYCells(this.resolution);

            for (var y = yCells.start; y <= yCells.end; ++y) {
              if (
                this.getWall(node.getCellLeft(node.xp, this.resolution), y) ||
                (y != yCells.start &&
                  this.getCeiling(node.getCellLeft(node.x, this.resolution), y))
              ) {
        ...
        if (node.onGround) {
          ...
          for (var x = xCells.start; x <= xCells.end; ++x) {
            ...
            if (
              this.getCeiling(
                x,
                node.getCellBottom(node.y, this.resolution) + 1
              ) ||
              (x != xCells.start &&
                this.getWall(
                  x,
      ...
      if (node.vy != 0) {
        ...
        if (node.vy > 0) {
          if (
            node.getCellBottom(node.y, this.resolution) !=
            node.getCellBottom(node.yp, this.resolution)
          ) {
            const xCells = node.getXCells(this.resolution);
            ...
            for (var x = xCells.start; x <= xCells.end; ++x) {
              if (
                ...
                (x != xCells.start &&
                  this.getWall(x, node.getCellBottom(node.y, this.resolution)))
              ) {
          ...
          if (
            node.getCellTop(node.y, this.resolution) !=
            node.getCellTop(node.yp, this.resolution)
          ) {
            const xCells = node.getXCells(this.resolution);

            for (var x = xCells.start; x <= xCells.end; ++x) {
              if (
                this.getCeiling(x, node.getCellTop(node.yp, this.resolution)) ||
                (x != xCells.start &&
                  this.getWall(x, node.getCellTop(node.y, this.resolution)))
              ) {
